...
const QuestionAnswerPairSchema = z.object({
  ...
  modelAnswer: z.string().describe("A \"Model Answer Guide\" for the interviewer, composed of MULTIPLE points (at least 3-4) to form a comprehensive checklist. Format this as a single string where each checklist point is separated by a double newline ('\\n\\n'). Each point MUST follow this format EXACTLY: A title for the evaluation point (e.g., 'Core Principles of OOP'), followed by a newline, and then a very detailed, multi-sentence explanation of that point. This explanation must educate a non-technical interviewer by explaining the concepts and substance of a strong answer, not just describing what the candidate should say. It MUST NOT be prefixed with 'Sample:-' or any other label."),
...
2.  **Generate Rich Model Answers**: For each question, the \`modelAnswer\` MUST contain a comprehensive checklist of 3-4 evaluation points. Each point MUST consist of a title followed by a newline and then a very detailed, multi-sentence explanation. CRITICAL: This explanation must educate a non-technical interviewer on the topic by explaining the core concepts and providing the substance of an excellent answer, not just describing it. For example, instead of writing "Explains data structures," write "A good answer would define data structures as ways to organize data for efficient access. They should mention key types like arrays (for indexed access), linked lists (for dynamic memory), stacks (LIFO), and queues (FIFO), and briefly explain the trade-offs." You MUST NOT use the word 'Sample' or any other prefix.
