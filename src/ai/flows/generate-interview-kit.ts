...
const QuestionAnswerPairSchema = z.object({
  ...
  modelAnswer: z.string().describe("A \"Model Answer Guide\" for the interviewer, composed of MULTIPLE points to form a comprehensive checklist. Format this as a single string where each checklist point is separated by a double newline ('\\n\\n'). Each point MUST follow this format EXACTLY: A title for the evaluation point (e.g., 'Explains Core Principles of OOP'), followed by a newline, and then 'Sample:- ' followed by a detailed, technically rich sample of a good answer. The sample should be comprehensive enough for a non-technical interviewer to understand what a strong answer sounds like and what key concepts to listen for. For example: 'Explains what is Java.\\nSample:- Java is a high-level, class-based, object-oriented programming language that is designed to have as few implementation dependencies as possible. It is known for its 'write once, run anywhere' (WORA) capability, meaning that compiled Java code can run on all platforms that support Java without the need for recompilation. Its key features include strong memory management, automatic garbage collection, and a rich API.'"),
