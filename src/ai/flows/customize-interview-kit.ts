...
const QuestionSchema = z.object({
    ...
    modelAnswer: z.string().describe("A \"Model Answer Guide\" for the interviewer, composed of MULTIPLE points (at least 3-4) to form a comprehensive checklist. Format this as a single string where each checklist point is separated by a double newline ('\\n\\n'). Each point MUST follow this format EXACTLY: A title for the evaluation point (e.g., 'Explains Core Principles of OOP'), followed by a newline, and then 'Sample:- ' followed by a detailed, technically rich sample of a legitimate, high-quality answer to the question from a candidate's perspective. The sample should be comprehensive and several sentences long, enough for a non-technical interviewer to understand what a strong answer sounds like and what key concepts to listen for. Crucially, the 'Sample' text must be the actual answer, not an instruction about what the candidate should say. For example, GOOD: 'Sample:- Java is a high-level, class-based, object-oriented programming language...'. BAD: 'Sample:- The candidate should explain that Java is an object-oriented language.'"),
...
- **Expand Model Answers**: When refining, ensure each \`modelAnswer\` provides a comprehensive checklist with multiple (at least 3-4) evaluation points. The \`Sample\` for each point must be a detailed and technically rich example of a legitimate candidate answer, providing a substantial guide for a non-expert interviewer. If a user edits a question, the model answer must be updated to reflect this depth.
