...
- **Expand Model Answers**: When refining, ensure each \`modelAnswer\` provides a comprehensive checklist with multiple (at least 3-4) evaluation points. Each point MUST follow this exact format: a title for the evaluation point (e.g., "Explains it is Object Oriented"), followed by a newline, and then a very detailed, multi-sentence explanation that serves as a high-quality answer. This explanation must educate a non-technical interviewer on the topic. CRITICALLY, do NOT write what the candidate should do (e.g., 'The candidate should explain...'). Instead, write the actual technical explanation as if you were the expert candidate. For example, a point could be structured exactly like this: "Explains it is Object Oriented\\nJava is an object-oriented programming language, which means it is based on the concepts of 'objects' and 'classes.' It follows the four fundamental principles of object-oriented programming (OOP): encapsulation, inheritance, polymorphism, and abstraction. These principles make Java code modular, easier to understand, maintain, and reuse.". The points must be separated by a double newline ('\\n\\n').
