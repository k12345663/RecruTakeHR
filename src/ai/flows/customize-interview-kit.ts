...
const QuestionSchema = z.object({
    ...
    modelAnswer: z.string().describe("A \"Model Answer Guide\" for the interviewer, composed of MULTIPLE points (at least 3-4) to form a comprehensive checklist. Format this as a single string where each checklist point is separated by a double newline ('\\n\\n'). Each point MUST follow this format EXACTLY: A title for the evaluation point (e.g., 'Explains Core Principles of OOP'), followed by a newline, and then 'Sample:- ' followed by an explicitly detailed explanation of the point itself. This explanation serves as a guide for the interviewer, detailing what concepts are involved and what constitutes a strong response. The sample should be written to educate a non-technical interviewer on the topic, while also serving as a benchmark for a high-quality candidate answer. Each sample must be several sentences long at a minimum, providing deep context and explanation. Crucially, the 'Sample' text must be the actual explanation and benchmark, not an instruction about what the candidate should say. For example, GOOD: 'Sample:- Object-Oriented Programming (OOP) is a programming paradigm based on the concept of 'objects', which can contain data and code. A strong answer would explain the four main principles: Encapsulation (bundling data and methods that operate on the data), Abstraction (hiding complex implementation details), Inheritance (allowing new classes to take on the properties of existing classes), and Polymorphism (allowing methods to do different things based on the object it is acting upon)...'. BAD: 'Sample:- The candidate should explain OOP principles.'"),
...
- **Expand Model Answers**: When refining, ensure each \`modelAnswer\` provides a comprehensive checklist with multiple (at least 3-4) evaluation points. The \`Sample\` for each point must be an explicitly detailed explanation of the point itself, designed to educate a non-technical interviewer on the topic. Each sample must be several sentences long at a minimum, providing deep context and explanation. If a user edits a question, the model answer must be updated to reflect this enhanced depth.
