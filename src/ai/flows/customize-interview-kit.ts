...
const QuestionSchema = z.object({
    ...
    modelAnswer: z.string().describe("A \"Model Answer Guide\" from the INTERVIEWER'S PERSPECTIVE. This is not a script for the candidate, but a checklist of 2-4 key points for the interviewer to look for in the candidate's response. Format this as a single string, with each checklist item on a new line (separated by '\\n'). Each item MUST be a complete sentence describing an expected concept or skill. It MUST end with a \"(Brief Answer Snippet: ...)\" explaining the core idea for a non-technical recruiter, and an \"(approx. X points)\" indicating its relative value. For example: 'Defines data structures such as arrays, linked lists, stacks, queues, trees, and graphs (Brief Answer Snippet: explaining their properties and use cases). (approx. 3 points)'"),
