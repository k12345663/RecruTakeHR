...
const QuestionSchema = z.object({
    ...
    modelAnswer: z.string().describe("A \"Model Answer Guide\" for the interviewer, composed of MULTIPLE points (at least 3-4) to form a comprehensive checklist. Format this as a single string where each checklist point is separated by a double newline ('\\n\\n'). Each point MUST follow this format EXACTLY: A title for the evaluation point (e.g., 'Core Principles of OOP'), followed by a newline, and then a very detailed, multi-sentence explanation of that point. This explanation must be updated to educate a non-technical interviewer by explaining the concepts and substance of a strong answer. It MUST NOT be prefixed with 'Sample:-' or any other label."),
...
- **Expand Model Answers**: When refining, ensure each \`modelAnswer\` provides a comprehensive checklist with multiple (at least 3-4) evaluation points. Each point MUST consist of a title followed by a newline and then a very detailed, multi-sentence explanation. CRITICAL: This explanation must educate a non-technical interviewer by explaining the core concepts and providing the substance of an excellent answer, not just describing what the candidate should do. For example, instead of "Explains data structures," write "A good answer would define data structures as ways to organize data for efficient access. They should mention key types like arrays (for indexed access), linked lists (for dynamic memory), stacks (LIFO), and queues (FIFO), and briefly explain the trade-offs." You MUST NOT use the word 'Sample' or any other prefix for the explanation. If a user edits a question, the model answer must be updated to reflect this enhanced depth.
