...
const QuestionSchema = z.object({
    ...
    modelAnswer: z.string().describe("A \"Model Answer Guide\" for the interviewer, composed of MULTIPLE points (at least 3-4) to form a comprehensive checklist. Format this as a single string where each checklist point is separated by a double newline ('\\n\\n'). Each point MUST follow this format EXACTLY: A title for the evaluation point (e.g., 'Explains Core Principles of OOP'), followed by a newline, and then a very detailed, multi-sentence explanation of that point. This explanation serves as a guide for the interviewer, detailing what concepts are involved and what constitutes a strong response, effectively educating a non-technical interviewer on the topic. It MUST NOT be prefixed with 'Sample:-' or any other label."),
...
- **Expand Model Answers**: When refining, ensure each \`modelAnswer\` provides a comprehensive checklist with multiple (at least 3-4) evaluation points. Each point MUST consist of a title followed by a newline and then a very detailed, multi-sentence explanation. This explanation must be so thorough that it educates a non-technical interviewer on the topic, providing deep context and a benchmark for a high-quality answer. You MUST NOT use the word 'Sample' or any other prefix for the explanation. If a user edits a question, the model answer must be updated to reflect this enhanced depth.
